...
class PriceListController extends Controller
{
    public function index(): RedirectResponse|View
    {
        if (config("price-list.singlePage")) {
            ...
            $tree = PriceListActions::buildNestedTree();
            ...
            if (! $priceList->show_nested) {
                ...
                if ($nestedChild->id !== $priceList->id) { $priceList = $nestedChild; }
            }
    ...
    public function show(PriceListInterface $priceList): RedirectResponse|View
    {
        ...
        if ($renderPriceTree) { $tree = PriceListActions::buildNestedTree($priceList); }
        else { $tree = $priceList->children()->select("id", "title", "slug")->orderBy("priority")->get(); }
